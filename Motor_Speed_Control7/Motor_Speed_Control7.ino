...
PID CONTROL(Ts, sigma, .5, .2, 0.001);
...
void setup() {
  ...
  lcdScreen.init2(1000000.0, 240000.0, capMeasure);
  lcdScreen.setBacklight(100, 100, 100, 255);
...
void loop()
{
  ...
  SUPPLY_VOLTAGE = lcdScreen.displayVolt2();
  //Serial.println(command);
  long currT = micros(); // in [us]
  command = BT.receive();
  if ((currT - prevT > Ts * 1e6))
  { 
...
//      CONTROL.readLimits(0, SUPPLY_VOLTAGE);
      ...
      if (deltaT >= Ts)
      {
        ...
        IMU.getAcc();
        yaw = IMU.pitch;//*DEG_TO_RAD;
        double yawDot_IMU = IMU.GyroY * DEG_TO_RAD;

        voltage1_enc = CONTROL.computePID(vel1, vel_D, Ts);
        voltage2_enc = CONTROL.computePID(vel2, vel_D, Ts);
//        double volt age = CONTROL.computePID(vel, vel_D, deltaT);
